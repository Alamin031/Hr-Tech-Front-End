...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const chckUser = await axios.get(`http://localhost:3000/admin/searchUser/${email}`);
      console.log(chckUser.data);
      if(chckUser.data === 'Customer'){
        try {
          const response = await axios.post('http://localhost:3000/customer/signin', 
        {
          email: email,
      ...
      response.data === 'Already Logged In an Account Please Logout First'
    ) {
      setError('Already Logged In an Account Please Logout First');
    }else {
